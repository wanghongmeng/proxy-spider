...
import cn.com.fero.tlc.spider.job.impl.TLCSpiderFetchProxyJob;
...
public class TLCSpiderValidateProxyJob extends TLCSpiderJob {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        if (isValidateProxy()) {
            ...
            TLCSpiderConstants.SPIDER_CONST_PROXY_STATUS = false;
            updateProxy();
        }
    }
    ...
    public static boolean isValidateProxy() {
        ...
        if (response.get(TLCSpiderConstants.SPIDER_CONST_RESPONSE_STATUS_CODE) == TLCSpiderConstants.SPIDER_CONST_RESPONSE_STATUS_SUCCESS
                && !response.get(TLCSpiderConstants.SPIDER_CONST_RESPONSE_CONTENT).toString().contains(IP_LOCALHOST)) {
